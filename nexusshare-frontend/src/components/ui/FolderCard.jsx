...
const FolderCard = ({ name, fileCount, size, colorClass, onRename, onDelete, view }) => {
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const isList = view === 'list';

  // --- LIST VIEW: Full Width, Reduced Height ---
  if (isList) {
    return (
      <div className="group flex items-center bg-white dark:bg-gray-800 p-3 px-6 rounded-2xl border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all cursor-pointer w-full relative">
  ...
  // --- GRID VIEW: Original Design (Large Card) ---
  return (
    <div className="folder-card bg-white dark:bg-gray-800 p-6 rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-xl group relative transition-all duration-300 hover:-translate-y-1">
...
const DropdownMenu = ({ onRename, onDelete, close }) => (
  <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-gray-700 rounded-xl shadow-2xl border border-gray-100 dark:border-gray-600 z-30 overflow-hidden shadow-indigo-500/10">
    <button 
